...
**모델 태그를 먼저 확인한다.** 지금 이 기계에 있는 것은 `qwen2.5:7b`
**하나뿐이다**(2026-09-07 `ollama list` 확인). 문서 예시·`start.bat`과 맞고,
`start.bat` 74~89줄의 `findstr /I "qwen2.5:7b"` 관문도 통과한다.

**이 줄은 기계 상태이므로 낡는다.** 2026-09-04 에는 정반대였다 — `7b` 가 없고
`qwen2.5:latest`·`qwen2.5:14b` 만 있어서 GUI 가 아예 뜨지 않았다. 없는 태그를
넘기면 02단계에서 멈추므로, 적혀 있는 것을 믿지 말고 `ollama list` 로 먼저
보고 부른다.
...
- **02단계가 절을 엉뚱한 기법에 붙이면 아무도 못 잡는다.** 지어낸
  `entities`(2026-09-05)와 통째로 빠뜨린 축(2026-09-04)은 닫혔지만, 옮기긴
  했는데 **잘못 붙인 것**은 인용이 있어 커버리지 검사가 침묵한다. 02 의 기법
