...
예전에는 스킵이 `errors.jsonl`에만 남아 07단계가 볼 수 없었고, 그 결과 보고서가 읽지 못한 아티팩트를 **언급조차 하지 않았습니다**(`docs/limitations-log.md` 4-1). `errors.jsonl`은 전 단계가 공유하는 집계용 로그라, 07이 그것을 파싱하면 에러 로그 형식에 묶입니다.
...
연속된 실패를 한 구간으로 묶는 이유는, 걸음마다 세면 비저널 구간 하나가 수만 건으로 부풀어 **"저널이 심하게 손상됐다"고 정반대로 읽히기** 때문입니다(실측 사례는 `docs/limitations-log.md` 4-0-1).
...
`06_verified.json`의 `passed`와 `unverifiable`을 구분해 생성합니다. `passed`는
...
| 시각 | 판정 | 사건 | 근거 |
|---|---|---|---|
| 2026-07-20 03:14:22 | Passed | shell.aspx 생성 | MFT#12345 |
| 2026-07-20 03:22:15 | Passed | svc_backup 계정 생성 | EVTX-SEC#40912 |

## 주의 필요 소견 (🟡 Warning - 분석가 교차 검증 요망)
웹셸을 통한 초기 침투 이후 계정 생성으로 지속성을 확보한 흐름으로 판단됨
> ⚠️ 분석가 확인 권장: claims 없음 (종합 판단 문장)
...
주의 필요 소견과 미확인 분석 범위를 보고서에 명시하는 것이 이 도구의
신뢰성 근거입니다. 자동 생성 시 누락되지 않도록 템플릿에 고정 섹션으로 둡니다.
