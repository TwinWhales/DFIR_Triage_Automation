...
먼저 잡혀 "레코드 0건"이 나왔습니다. `limitations-log.md` 4-0에 원인과 조치를
적었고, 아래는 고친 뒤의 결과입니다.
...
→ 파서는 정확히 동작했습니다(가짜 레코드 0건). 다만 8바이트 스캔 1회를
  ...
  로 고쳤습니다(`limitations-log.md` 4-0-1).
...
실물 Win10 증거가 들어오면 다시 대조해야 합니다(`limitations.md` 2장).
...
05단계로 갑니다. `limitations.md` 3-2가 `$UsnJrnl`에 대해 적은 것과
...
`docs/limitations.md` 2-3이 그 기록입니다.
...
`limitations.md` 2-10에 *"부모 MFT 엔트리가
...
내려가 본 적이 없었다는 뜻입니다(`docs/limitations-log.md` 2026-08-28).
...
`docs/limitations.md` 2-9이 든 수치는 전부
...
## 2026-09-06 · 장치 속성을 DEVPROP 으로 읽었다 — 전/후 대조

`work.md` 14번을 구현하며 잰 값입니다. 대상은 키오스크 KAPE 수집
(`evidence/KAPE_Results/snapshot1/C`) 의 SYSTEM 하이브이고, 03단계 선별이
`Enum\USB` 를 지목하므로 그 서브트리가 전부입니다.

### `Properties` 아래에 모양이 둘이다 — 경계를 먼저 그었다

2026-09-03 절이 "경로로 켜라" 고 적었는데, **어느 경로인가**를 이번에
실물로 갈랐습니다. 같은 하이브 안에 둘이 있습니다.
...
B 는 `Control\Class\{…}\Properties\Security`·`…\DeviceType` 같은
것들이고 **지금 옳게 읽힙니다.** "경로에 `Properties` 가 있으면" 으로 켜면
이 71건을 깨뜨립니다. 그래서 `{GUID}\NNNN` 까지 맞아야 켭니다.

A 의 값 이름은 6,208건이 `(default)` 이고 18건이 `ko-KR` 입니다(로케일별
FILETIME). 이름이 아니라 **키 경로**로 판정하는 근거이기도 합니다.

### 전/후 — 레코드 303건, `Enum\USB` 전량
...
남은 1건은 값이 하나도 없는 키입니다 — 읽지 못한 것이 아니라 애초에 없습니다.
**세는 단위를 디코더가 켜지는 경로로 맞췄습니다.** 경로에 `Properties` 가
들어가면 세는 식으로 잡으면 상위 키(`…{GUID}` 자신, `…\Properties` 자신)가
섞여 분모가 커집니다.

값이 바뀐 자리를 타입별로 가르면 이렇습니다.

| 타입 | 건수 | 전 → 후 |
|---|---:|---|
| `0x12` STRING | 109 | `null`(값 없음) → `"USB Input Device"`·`"input.inf"`·`"HID_Inst.NT"` |
| `0x10` FILETIME | 41 | **전부 그대로** ← 회귀 감시 자리 |
| `0x11` | 8 | `255` → `"ff000000"` |
| `0x07` | 30 | `[]` → `""` (둘 다 빈 값) |

**`0x10` 이 안 움직인 것이 이 표에서 가장 중요합니다.** 장치의 연결·제거
시각이 그 타입이고, 표준 번호와 우연히 같아 전부터 옳게 읽히고 있었습니다.
새 경로가 그것을 건드렸다면 USB 타임라인이 통째로 흔들립니다.

### GUID 는 실물 표본이 검산해 주었다

`0x0D` 는 "길이가 전부 16바이트" 라는 생김새만으로 GUID 로 본 것이라
근거가 약한 축이었습니다. 리틀엔디언으로 조립해 보니 표본이
**실재하는 장치 GUID** 로 떨어집니다.
...
바이트 순서가 틀렸다면 이런 모양이 나오지 않습니다. 시험에 이 값을 못
박아 두었습니다.

### 안 읽는 것 둘 — 그대로 두었다

- `0x11` (919건, 전부 4바이트 `ff000000`) — 명세의 `DEVPROP_TYPE_BOOLEAN`
  은 1바이트입니다. **폭이 다르므로 단정하지 않습니다.**
- `0x82` (4건) — 구조를 모릅니다. 앞이 6개의 32비트 정수처럼 보이고 뒤에
  UTF-16 문자열이 붙습니다(`Intel(R) 82574L Gi…`).

**`0x11` 에 실마리가 하나 생겼습니다.** 4바이트라는 폭이 값의 진짜 폭이
아니라 **셀 인라인 저장의 폭일 수 있습니다.** vk 레코드는 4바이트 이하 값을
셀 안에 직접 담는데, 그 자리를 길이대로 자르지 않고 통째로 돌려주면 1바이트
`ff` 가 `ff000000` 으로 보입니다. 같은 설명이 `0x05` 의 `0500ffff` 에도
걸립니다. **확인하지 않았습니다** — 선언된 데이터 길이를 vk 에서 직접 읽어
대조해야 하고, 그때 이 둘이 함께 풀릴 수 있습니다.

### 재현

```bash
A=evidence/KAPE_Results/snapshot1/C
.venv/Scripts/python.exe -m src.stage04_parse.parse \
  --in <선별> --out <출력> --evidence "$A" --volume 1
# 타입 히스토그램은 value_type() 을 쓴다 — value_type_str() 은 예외를 던진다
```

---

## 2026-09-07 · `$UsnJrnl` v3 를 읽게 했다 — 명세 대조와, 실물에서 잰 것 둘

`work.md` 2번의 앞쪽(v3/v4 보강)입니다. **v3 는 실물이 없어 명세만 대조했고,
대신 그 작업에서 나온 코드 변경을 실물 저널 둘로 전/후 대조했습니다.**

### v3 레이아웃 — 대조한 것은 명세 둘뿐이다

| 출처 | 무엇을 확인했나 |
|---|---|
| MSDN `USN_RECORD_V3` (`winioctl.h`) | 필드 순서·폭·헤더 76바이트 |
| `[LIBFSNTFS]` rev 0.0.28 | 같은 표. 두 출처가 일치 |
| 우리 코드 | `struct.calcsize("<IHHQQQQQQIIIIHH") == 76` 을 테스트로 고정 |

**외부 도구 대조도, 실물 대조도 없습니다.** NTFS 볼륨의 온디스크 `$J` 는
V2 로 기록되고, V3 가 나오려면 ReFS 이거나 범위 추적을 켠 볼륨이어야 하는데
손에 그런 이미지가 없습니다. 이 기계의 저널 둘(626,600레코드)은 **전부
v2** 입니다 — `unsupported_version` 이 0 인 것이 그 뜻입니다.

프리패치 표에서 배운 것이 정확히 이 자리입니다(`work.md` 1번) — **명세를
옮긴 줄은 실물에 반증될 수 있습니다.** 그래서 v3 로 읽은 레코드는
`fields.usn_record_version` 을 달고 나갑니다. 나중에 그 레코드들만 골라
되짚을 수 있어야 하기 때문입니다.

`file_entry`·`file_sequence` 를 무엇으로 채울지가 v3 를 막고 있던 것인데
(`docs/limitations.md` 의 옛 절), **NTFS 한정으로는 막힌 것이 아니었습니다.**
`FILE_ID_128` 은 64비트 파일 참조를 하위 8바이트에 담고 상위 8바이트를 0 으로
둡니다. 상위가 0 이 아니면 NTFS 참조가 아니므로(ReFS) 레코드를 내지 않고
건너뜁니다 — 하위만 잘라 담으면 **없는 MFT 엔트리를 가리키는 레코드**가 됩니다.

### 실물에서 잰 것 ① — 전/후가 완전히 같다

대상은 키오스크 KAPE 수집 둘의 `$Extend/$J` (39.4MB · 33.0MB) 입니다.

| | snapshotA | snapshotB |
|---|---:|---:|
| 레코드 | 341,086 | 285,514 |
| `parse_errors` · `unreadable_bytes` | 0 · 0 | 0 · 0 |
| `unsupported_version` | 0 | 0 |
| USN 집합 해시 | **전/후 동일** | **전/후 동일** |

건수만이 아니라 **어느 USN 이 나왔는지의 집합까지 같습니다.** 건수가 같으면서
내용이 바뀌는 경우를 가르기 위해 집합을 따로 비교했습니다.

### 실물에서 잰 것 ② — 레코드는 4096 경계를 넘지 않는다

같은 작업에서 **청크 경계에 걸친 레코드를 손상으로 세는 버그**를 찾았습니다
(전말은 `docs/limitations-log.md` 2026-09-07 절). 합성 저널에서는 재현되는데
위의 실물 둘에서는 전/후가 같았습니다. **왜 안 터졌는지를 쟀습니다.**

| | snapshotA | snapshotB |
|---|---:|---:|
| 4096바이트 경계를 넘는 레코드 | **0** | **0** |
| 레코드 길이 최소/최대/평균 | 64 / 376 / 119.4 | 64 / 376 / 116.7 |
| 청크 경계에 걸친 레코드 | **0** | **0** |

626,600건 전부가 한 페이지 안에 들어 있습니다. NTFS 가 `$J` 레코드를 페이지
경계 너머로 걸치지 않게 쓰고 남는 자리를 0 으로 채우기 때문이고, 파서가 세는
`zero_bytes_skipped`(64.9만 · 49.5만 바이트) 의 상당 부분이 그 패딩입니다.

**기본 청크 1 MiB 는 4096 의 배수라 버퍼 경계가 항상 페이지 경계와 겹칩니다.**
그래서 실물 NTFS 저널에서는 이 버그가 도달 불가능했습니다. 도달하는 조건은
셋입니다 — 청크 크기가 페이지 배수가 아닐 때(파서는 8192 이상이면 받습니다),
카빙·재조립으로 페이지 정렬이 깨진 `$J`, 그리고 **v3**(헤더가 76바이트라
옛 예비 검사값 60 을 넘습니다).

**이 표는 "고쳐서 레코드를 되찾았다"가 아닙니다.** 이 저널들에서는 잃은 것이
없었고, 그 사실을 확인한 것이 이 측정의 결과입니다.

### 재현

```bash
J='evidence/KAPE_Results/KIOSK_snapshotA_20260831T094536/C/$Extend/$J'
# 홑따옴표가 필요하다 — 셸이 $Extend·$J 를 변수로 읽는다
.venv/Scripts/python.exe -m pytest tests/test_usn_parser.py -q
```

전/후 대조는 `git checkout -- src/stage04_parse/` 로 옛 코드를 되돌려 같은
...
합성 레코드가 아니라 **이 증거의 그 한 건**으로 03 → 04 → 05 배분까지
태웠습니다. 시나리오는 `T1070.001` 하나만 담아 손으로 만들었고, 시간창은
픽스처의 것(`2026-07-18~22`)을 그대로 두었습니다 — **창 밖이어도 도달하는지가
확인점**이기 때문입니다.

| 단계 | 결과 |
|---|---|
| 03 선별 | `evtx:Security`(`event_ids: [1102]`) · `evtx:System`(`[104]`) 둘, `evtx:Sysmon` 은 유예 |
| 04 파싱 | `evtx:Security` **1건**(플래그 1건) · `evtx:System` 0건 |
| 04 플래그 | `['log_cleared', 'outside_time_range']` |
| 05 배분 | **전달됨** — 창 밖인데도 |

마지막 줄이 두 변경이 맞물리는 자리입니다. `window_independent` 가 없었다면
이 레코드는 `outside_time_range` 때문에 신호끼리의 경쟁에서 뒤로 밀립니다.
...
## 2026-09-07 · ValleyRAT KAPE의 `$J`를 `$MFT` 원본과 대조했다

대상은 `evidence/ValleyRAT-kape/C/$Extend/$J`다. 처음에는 희소 파일이나
USN 헤더 변형 때문에 310,120개 구간이 실패한 것으로 보였지만, 바이트
대조 결과 파일 종류 자체가 달랐다.

| 항목 | 측정값 |
|---|---|
| 대상 크기 | 35,259,682바이트 |
| 시작 바이트 | `06 00 46 49 4c 45 30 00` — 오프셋 2에서 `FILE0` |
| 반복 간격 | 1,024바이트 |
| 첫 세 레코드 번호 | 436,917 · 436,918 · 436,919 |
| 원본 대조 위치 | 같은 수집본 `$MFT`의 447,403,008바이트부터 |
| CopyLog 원본 논리 크기 | 569,665,824바이트 |
| CopyLog/실제 대상 SHA1 | `D5A3A6FBADFC806201901BD578FAEC1A5FFA86D0` / 일치 |

첫 20개 고정 레코드를 같은 수집본 `$MFT`의 해당 위치와 비교했고 전부
바이트 단위로 같았다. CopyLog 해시도 현재 대상과 같으므로 파서가 파일을
잘못 연 것이 아니라 KAPE 산출 단계에서 `$J` 이름에 `$MFT` 조각이
들어온 것이다.

조치는 `usnjrnl.py`의 사전 검사다. 최대 7바이트 접두어를 허용해 세 개의
`FILE` 시그니처가 1,024바이트 간격이고 레코드 번호가 연속일 때만
오수집으로 판정한다. 우연히 USN 파일명 본문에 `FILE` 문자열이 든 경우는
이 조건을 통과하지 않는다. 이 파일에서 USN 사건을 만들어 내지 않고,
재수집이 필요하다는 오류를 즉시 낸다.
