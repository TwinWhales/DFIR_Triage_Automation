...
`passed`는 녹색, `unverifiable`은 노란색으로 실립니다. `rejected`는 본문과
타임라인에서 차단됩니다. 다음 고정 섹션이 반드시 들어갑니다.

- **확인된 사실 (🟢 Passed)** — claims와 서술이 증거에 부합
- **주의 필요 소견 (🟡 Warning)** — `unverifiable` 소견, 근거와 수동 검증 사유 보존
- **미확인 사항 (⚪ Unknowns)** — `excluded` + 발동하지 않은 `deferred` 등 범위 한계
...
**`--llm ollama` 경로는 2026-08-31에 실물로 통과했습니다.** `qwen2.5:latest`로
02는 C-001 서술에서 18초에 기법 2건을 스키마대로 냈고, 05는 목업 레코드
5건에서 30.8초에 findings 4건을 내 06 검증을 전부 통과했습니다(환각률 0.0%).

그 실행에서 **출력 상한이 없다는 것**이 드러났습니다. 실물 이미지로 돌린
알럿 케이스(레코드 60건, 프롬프트 16,330토큰)에서 05가 300초 타임아웃에
세 번 걸려 중단됐는데, 생성 속도는 27~39 tok/s여서 정상 응답이라면 1분이면
끝날 양이었습니다. 원인은 `num_predict`를 안 보내 Ollama 기본값(`-1`,
EOS까지)이 걸린 것이었습니다. 대개는 멈추지만 **안 멈출 때 바닥이 없습니다**
— 같은 프롬프트를 상한 없이 두고 35분을 기다려도 응답이 오지 않았고, 상한을
걸자 37.7초에 끝났습니다.

지금은 각 단계가 자기 상한을 넘깁니다. 02는 `DEFAULT_NUM_PREDICT`(2048,
실측 출력 233토큰의 여덟 배), 05는 `--reserve-output-tokens`를 그대로
`num_predict`로 보냅니다 — 예산에서 비워 둔 자리와 실제로 쓸 수 있는 양이
같은 수여야 예산 계산이 가정이 아니라 약속이 됩니다.

상한에 닿아 잘린 응답은 `malformed_output`으로 기록되며, `extract_json`이
"응답이 중간에 잘림"으로 형식 위반과 구별해 적습니다. 프롬프트를 고쳐야 할
일과 상한을 올려야 할 일은 대응이 정반대이므로 같은 문구로 두지 않습니다.
...
`options`에 실리는 값 중 둘은 **주지 않으면 대가가 조용히 옵니다.** `num_ctx`를
빼면 서버 기본값(4096)으로 모델이 떠 프롬프트가 말없이 잘리고, `num_predict`를
빼면 멈추지 않는 응답을 붙잡을 것이 타임아웃밖에 없습니다. 값은 단계가 정하고
전송 계층은 받기만 합니다.
