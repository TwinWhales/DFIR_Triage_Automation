...
설명에서 6주째 가리키고 있던 0바이트 파일이다. 실물 이미지에서 **7종
280건을 되짚었고 어긋남 0건**이다. 파서로 다시 파싱해 비교하는 것이 아니라
레코드가 자기 안에 들고 있는 식별자로 대조한다 — `$MFT` 는 업데이트
시퀀스까지 본다. 남은 것 둘(SRUM 을 실물로 못 잰 것, 프리패치는 오프셋으로
가릴 것이 없는 것)은 `docs/limitations.md` 의 같은 절에 있다.
