...
## 6. evtx 파서 엔진 — 재 봤다. 이득은 진짜고 조건이 붙는다
...
**결정적이었던 것은 처리량이 아니라 청크 복구다.** System.evtx 는 헤더가
선언한 것 밖에 청크가 하나 더 있고, 우리 파서는 그것을 살려 1,268건을
낸다(python-evtx 를 그대로 쓰면 1,238건). pyevtx-rs 도 **1,268건**이다.
이게 안 됐으면 아무리 빨라도 못 썼다.

`json.loads` 까지 포함해 잰 값이고 우리 파싱 단계의 약 215배다.

### 그런데 끝단 이득은 215배가 아니라 2.75배다
...
파싱이 0 이 되어도 1.30초가 남는다. 10만 건이면 9.4분 → 3.5분이다.

### 채택 비용의 본체 — 두 패키지는 Windows 에서 공존할 수 없다
...
**이것이 부수적 불편이 아니라 이 항목의 가장 큰 비용이다.**

- 점진 이관이 안 된다. 한 번에 갈아타야 한다
- 한 프로세스 안에서 A/B 대조가 안 된다 — 대조 하네스가 환경을 갈라야 한다
- 팀 전원과 CI 가 동시에 갈아타야 한다
- 되돌릴 길이 `requirements.txt` 롤백뿐이다

### 그래서 집을 때 함께 해야 할 것 넷

1. **`RuntimeError: Failed to parse chunk header` 를 삼키지 않는다.**
   Security·Application 에서 마지막 유효 레코드 **뒤에** 난다. 레코드 ID
   집합이 우리·wevtutil 과 정확히 같으니 손실은 없지만, 이것을 "끝"으로
   처리하면 **진짜 손상과 구분이 안 된다.** "폴백을 만들지 않는다"와
   정면으로 만나는 자리다
2. **공존 불가를 `docs/limitations.md` 에 적고** `requirements.txt` 에서
   python-evtx 를 뺀다
3. **대조 절차를 문서로 남긴다.** 환경이 갈라져 CI 에 못 들어간다
4. **선언 밖 청크 복구를 회귀 테스트로 고정한다.** System.evtx 가 1,238
   이 아니라 1,268 이어야 한다는 것 자체가 합격 기준이다

### 순서 — 파서 성능 트랙의 마지막

앞의 flagging·`$MFT` 작업이 위 표의 **36%** 를 줄인다. 그것이 끝난 뒤에
다시 계산해야 교체 이득이 제대로 보인다. 되돌리기 어려운 변경을 먼저
하지 않는다.

재현은 격리해서 한다 — 프로젝트 `.venv` 에 깔면 안 된다.
