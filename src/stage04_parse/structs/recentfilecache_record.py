...
``0x08``          4   미상 (실측 ``3``)
``0x0C``          4   미상 (실측 ``1``)
``0x10``          4   미상 (실측 ``0x0178A0D1``)
==============  ====  ====================================================

실측값은 `evidence/windows7_testimage.001`(Windows 7 Ultimate, 빌드 7601)
하나에서 본 것입니다. **항목 수(67)와 맞는 값이 없으므로 개수 필드는
없습니다** — 그래서 파일 끝까지 걸어야 합니다.
...
## 대조 상태 — 실물 1건으로 확인했다 (2026-08-25)

`evidence/windows7_testimage.001`(Windows 7 Ultimate, 빌드 7601) 볼륨 1의
실제 파일 6,742바이트로 대조했습니다. **위 구조가 그대로 맞습니다.**

- 시그니처 `FE FF EE FF` 일치
- 길이 필드가 **문자 수**가 맞다 — 67개 항목을 연쇄해 걸었을 때 마지막
  항목의 끝이 파일 크기와 **정확히 같습니다**(남은 바이트 0). 단위를
  바이트로 잘못 잡았다면 첫 항목에서 어긋납니다.
- `tools/scan_recentfilecache.py`가 **길이 필드를 아예 보지 않고** 널로
  쪼갠 결과와 67건 전부 일치했습니다.

`docs/artifact-notes.md` "2026-08-25 · RecentFileCache.bcf 실물 대조" 참고.

**아직 남은 것** — 외부 도구(`RecentFileCacheParser`)와 맞춰 보지
않았습니다. 위 둘은 우리가 만든 두 경로라, 공통 오해가 있다면 둘 다
같은 방향으로 틀립니다. 다만 파일 크기는 Windows가 쓴 값이라 그 대조는
독립적입니다.

아래 검사들은 그대로 둡니다. 다른 빌드에서 구조가 달라질 수 있고,
어긋나면 그럴듯한 값을 내는 대신 **거부하는** 편이 낫습니다.
...
실측한 이미지는 **하나**입니다. 다른 Win7 빌드·언어·SP 조합에서 헤더가
다를 수 있고, 그때는 위 검사들이 걸러 냅니다.
"""
...
def read_header(data: bytes) -> None:
    ...
    if data[:4] != SIGNATURE:
        raise RecentFileCacheError(
            ...
            "대조한 실물은 Windows 7 Ultimate 빌드 7601 하나뿐이라 다른 "
            "빌드·언어·SP 조합에서 헤더가 다를 수 있습니다."
        )
