...
#: ``ungrounded_technique`` · ``timezone_adjusted`` 는 2026-09-07 에 추가됐다
#: (전체 공지 대상). 위의 셋과 같은 무리다 — **실패가 아니라 측정**이라
#: 조치가 ``record`` 다. 둘 다 `518_Test_0907` 실측에서 나왔고, 그 실행은
#: 11개 관문을 전부 PASS 하고도 정답 레코드를 놓쳤다.
#:
#: - ``ungrounded_technique`` — 02단계가 **이름 붙은 메커니즘을 전제하는
#:   세부 기법**을 골랐는데 입력 서술에 그 메커니즘이 한 번도 안 나왔다.
#:   실측: 입력이 "실행 방법은 아직 확인되지 않았습니다" 라고 명시했는데
#:   모델이 ``T1059.003``(Windows Command Shell)을 골랐고, 그 매핑이
#:   ``prefetch`` 를 Tier 2 로 두는 바람에 03단계가 프리패치를 유예했다.
#:   Tier 2 루프백이 없어 영구 미수집이 됐고, ``518.EXE-71CD1C31.pf`` 에
#:   있던 3회 실행 기록이 보고서에 한 줄도 오르지 않았다.
#:   ``ungrounded_entity`` 와 성격이 같다 — 지어낸 값이 뒤 단계의 **선별
#:   기준**이 되는 것을 막는 것이고, 02단계가 그것을 떨구고 여기 적는다.
#: - ``timezone_adjusted`` — 한국어 서술의 벽시계 시각을 모델이 그대로
#:   ``Z`` 로 옮겨, KST 로 읽은 값이 ``time_range`` 밖에 놓였다. 실측:
#:   ``오전 11시경`` → ``11:00:00Z``. 실제 실행은 ``01:57:30Z`` 였고
#:   아홉 시간 어긋난 범위 밖이라 ``outside_time_range`` 로 밀려났다.
#:   02단계가 범위를 넓히고 여기 적는다(`timeband` — 옮기지 않고 넓힌다).
#:
#: 세는 이유는 앞의 셋과 같다 — **프롬프트를 고쳤을 때 나아졌는지 알기
#: 위해서**다. 둘 다 프롬프트에 지시를 넣어 두었고, 소형 모델이 그것을
#: 지키기 시작하면 이 수치가 0 으로 간다.
#: ``investigation_rejected`` 는 2026-09-09 에 추가됐다(전체 공지 대상).
#: 위의 넷과 같은 무리다 — **실패가 아니라 측정**이라 조치가 ``record`` 다.
#:
#: 05단계가 낸 추가 조사 요청을 ``stage02_normalize.expand`` 가 받아들이지
#: 않은 것이다. 근거로 든 ``ref`` 가 실제로 전달한 레코드가 아니거나,
#: 존재하지 않는 ATT&CK ID 이거나, 매핑 YAML 이 없어 03단계가 아무것도
#: 고르지 못하거나, 파서가 없는 아티팩트이거나(`psreadline_history`),
#: 넓히자는 범위가 이미 1차 범위 안이거나 증거 수집 시각 뒤인 경우다.
#: 사유 어휘는 ``schemas/investigation.schema.json`` 의 ``disposition.reason``
#: 이 유일한 출처이며, 여기 옮겨 적지 않는다.
#:
#: ``stage`` 는 ``05_interpret`` 으로 적는다. 발견하는 코드는 02단계에
#: 있지만 **틀린 것은 05단계의 산출물**이고, 고칠 자리도 05의 조사 요청
#: 프롬프트다. ``by_stage_type`` 을 읽는 사람이 어디를 봐야 하는지가
#: 이 값으로 정해진다.
#:
#: 세는 이유는 앞의 넷과 같다 — **프롬프트를 고쳤을 때 나아졌는지 알기
#: 위해서**다. 열거형으로 묶어 두었으므로(요청 가능한 ``ref``·기법·
#: 아티팩트가 전부 enum 이다) 제약을 건 실행에서 이 수는 0 에 가까워야
#: 한다. 0 이 아니면 열거형이 새는 것이고, 그것은 모델 사정이 아니라
#: 우리 회귀다.
